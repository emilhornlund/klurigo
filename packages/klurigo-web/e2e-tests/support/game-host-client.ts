...
export class GameHostClient {
  ...
  /**
   * Opens the host's public game-event stream.
   *
  ...
  /**
   * Idempotently closes the host's SSE connection and rejects pending waits.
   */
  public close(): void {
  ...
  private request(
    ...
    const headers: Record<string, string> = {
      ...
      'User-Agent': SIMULATED_HOST_USER_AGENT,
    }
  ...
  private requireSession(): { accessToken: string; gameId: string } {
    if (!this.accessToken || !this.gameId) {
      throw new Error('Authenticate as a host before using this client')
    }
  ...
  private ensureOpen(): void {
    if (this.closed) {
      throw new Error('Game host client is closed')
    }
  }
